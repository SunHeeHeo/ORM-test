...
const getDetailPosts = async (req, res, next) => {
    ...
    try {
      ...
      //산책 동행인 닉네임 불러주는 코드 
      const company  = `SELECT notification.senderNickname from notification 
      where notification.postId =${postId} and notification.type = 3`
      let name = await db.query(company)
      let companyName= []
      for (let i=0; i<name.length; i++) {
        companyName.push(name[i].senderNickname)
      }
      //산책 상세 정보 
      const query =
